package com.mayousheng.www.minify.task

import com.mayousheng.www.minify.ext.MinifyConfig
import com.mayousheng.www.minify.template.ManifestTemplate
import com.mayousheng.www.minify.template.ResTemplate
...
class MinifyTask extends DefaultTask {
    ...
    @Nested
    MinifyConfig config = new MinifyConfig()
    ...
    int getRandomInt(int data) {
        int result = data
        if (result > 1) {
            result = result / 2 + new Random().nextInt(result / 2)
        }
        return result;
    }
    ...
    void generateActivity(String packageName, String activityPreName) {
        ...
        if (!config.excludeActivityJavaFile) {
            ...
            typeBuilder.addMethod(MethodSpec.methodBuilder("onCreate")
                    ...
                    .build())
            //其它方法
            for (int j = 0; j < getRandomInt(config.methodCountPerClass); j++) {
